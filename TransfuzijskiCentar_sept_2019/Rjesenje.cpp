...
ostream &operator<<(ostream &c, const OznakaKrvneGrupe &k) {
	if (k == A)c << "A";
	else if (k == B)c << "B";
	else if (k == AB)c << "AB";
	else if (k == 0)c << "0";
...
public:
	...
	int getDays() {
		return _dan + _mjesec * 30 + _godina * 30 * 12;
	}
};
...
public:
	...
	void Sort(string redoslijed) {
		if (redoslijed == "ASC" && _keyElement == 1) {
		...
		if (redoslijed == "DESC" && _keyElement == 1) {
			for (int i = 0; i < *_trenutnoElemenata - 1; i++)
			{
				for (int j = i + 1; j < *_trenutnoElemenata; j++)
				{
		...
		if (redoslijed == "ASC" && _keyElement == 2) {
			for (int i = 0; i < *_trenutnoElemenata - 1; i++)
			{
				for (int j = i + 1; j < *_trenutnoElemenata; j++)
				{
		...
		if (redoslijed == "DESC" && _keyElement == 2) {
			for (int i = 0; i < *_trenutnoElemenata - 1; i++)
			{
				for (int j = i + 1; j < *_trenutnoElemenata; j++)
				{
...
public:
	...
	bool operator==(const KrvnaGrupa &k) {
		return k._oznaka == _oznaka && k._rhFaktor == _rhFaktor;
	}
	string ToString() const {
		...
		*/
		stringstream ss;
		ss << "Krvna grupa ->" << _oznaka << _rhFaktor << crt;
		ss << "Donori ->";
		for (size_t i = 0; i < _donori.size(); i++)
		{
			if (i != _donori.size() - 1) {
				ss << _donori[i]._oznaka << _donori[i]._rhFaktor << ", ";
			}
			else
				ss << _donori[i]._oznaka << _donori[i]._rhFaktor;
		}
...
class Donor :public Osoba {
	...
	bool ValidanFormat(string broj) {
		string pravilo("([0-9]{3})([/-]{1})([0-9]{3})([-]{1})([0-9]{3})");
...
public:
	Donor(const char* imePrezime, KrvnaGrupa krvnaGrupa, string telefon, Datum dpd, bool remind = true, bool contact = true)
		:Osoba(imePrezime, krvnaGrupa), _telefon(telefon), _datumPosljednjegDoniranja(dpd), _podsjetiMe(remind), _kontaktirajMe(contact) {
	...
	friend ostream &operator<<(ostream &c, const Donor &d) {
		c << "Ime i prezime -> " << d._imePrezime << endl;
		c << "Broj telefona -> " << d._telefon << endl;
		c << "Datum zadnjeg doniranja -> " << d._datumPosljednjegDoniranja << endl;
		c << crt;
		return c;
	}
	void Info() {
	...
	bool operator==(const Donor &d) {
		return d._imePrezime == d._imePrezime || d._telefon == _telefon;
	}
};
...
public:
	...
	void SetKolicina(double kol) { _kolicina = kol; }
};
...
public:
	...
	bool AddDonaciju(Datum d, Osoba *o, double kolicina) {
		...
		bool imaNaStanju = false;
		for (size_t i = 0; i < _zalihe.GetTrenutno(); i++)
		{
			if (*_zalihe.GetElement1(i) == o->GetKrvnaGrupa()) //ima vec ova krvna grupa na stanju, znaci dodajemo joj novu kolicinu.
			{
				_zalihe.SetElement2(i, _zalihe.GetElement2(i) + kolicina);
				imaNaStanju = true;
			}
		}
		//ukoliko nije prije bilo ove krvne grupe na stanju, dodajemo novu zalihu.
		if (!imaNaStanju)
			_zalihe.AddElement(new KrvnaGrupa(o->GetKrvnaGrupa()), kolicina);
		//sada, provjeravamo da li je ova osoba prije donirala. ako jeste, dodajemo joj novu kolicinu doirane krvi.
		//Da bismo to provjerili, potrebna nam je klasa Donor, a implementirano imamo samo Osobu. Dynamic cast.
		Donor *tempOsoba = dynamic_cast<Donor*>(o);
		for (size_t i = 0; i < _donacije.GetTrenutno(); i++)
		{
			if (_donacije.GetElement1(i) == o) {	//ova osoba je vec donirala. Dodajemo novu kolicinu
		...
		//ako ova osoba nije nikada prije donirala, onda je dodajemo kao novog donatora.
		_donacije.AddElement(new Donor(*tempOsoba), kolicina);
		return true;
	}
	Datum *GetDatumPosljednjegDoniranja(string imePrezime, string brojTelefona) {
		...
		*/
		...
		for (size_t i = 0; i < _donacije.GetTrenutno(); i++)
		{
			if (_donacije.GetElement1(i)->GetImePrezime() == imePrezime || dynamic_cast<Donor*>(_donacije.GetElement1(i))->GetTelefon() == brojTelefona) {
				datum = &dynamic_cast<Donor*>(_donacije.GetElement1(i))->GetDatumPosljednjegDoniranja();
			}
		}
		return datum;
	}
	bool DodajZahtjev(Zahtjev z) {
		...
		bool postojiZahtjevanaKolicinaKrvi = false;
		for (size_t i = 0; i < _zalihe.GetTrenutno(); i++)
		{
			//trebamo otvoriti forpetlju i za primaoce, ali iz zahtjeva koji je implementiran, da bismo mogli provjeriti o kojoj se krvnoj grupi radi, pa tek onda da li ima kolicinski dovoljno.
			for (size_t j = 0; j < z.GetKrvnaGrupa().GetPrimaoci().size(); j++)
			{
				if (*_zalihe.GetElement1(i) == z.GetKrvnaGrupa().GetPrimaoci()[j] && _zalihe.GetElement2(i) >= z.GetKolicina()) {
					...
					postojiZahtjevanaKolicinaKrvi = true;
					return true;
				}
			}
		}
		if (!postojiZahtjevanaKolicinaKrvi) {
			cout << "Nemamo zaliha krvne grupe " << z.GetKrvnaGrupa().GetOznakaKrvneGrupe() << endl;

			thread jedan([this, &z]() {
				m.lock();
				//sada trazimo donatore, po datim uslovima...
				for (size_t i = 0; i < this->GetDonacije().GetTrenutno(); i++)
				{
					//posto ce nam trebati krvna grupa i telefon, moramo koristit dynamic_cast tj jer su ti atributi u Donoru a ne u osobi.
					Donor* donor = dynamic_cast<Donor*>(this->GetDonacije().GetElement1(i));
					for (size_t j = 0; j < donor->GetKrvnaGrupa().GetDonori().size(); j++)
					{
						if (donor->GetKrvnaGrupa().GetDonori()[j] == z.GetKrvnaGrupa() && donor->getKontakt() && (donor->GetDatumPosljednjegDoniranja().getDays() - z.GetDatumZahtjeva().getDays() >= 90)) {

							cout << "Saljem sms na broj telefona -> " << donor->GetTelefon();
	...
	void PrintajZahvalnice(int top) {
		//printa zahvalnice (zahvaljujemo se ime i prezime donoru na ukupno doniranih X doza krvi) za TOP 2 donatora krvi
		//prvo moramo negdje pohraniti sve donatore, tj donacije, a posto je u ovoj klasi vec implementirano, koristit cemo to.
		Kolekcija<Osoba*, double, 100> kopija(_donacije);
		//kako bismo nasli ko je donirao najvise krvi, moramo sortirati od najvecoj prema najmanjoj kolicini donirane krvi.
		//po defaultu je keyElement postavljen na T1, u ovom slucaju OSOBU. Nama treba sortiranje po KOLICINI(double).
		kopija.SetKeyElement(2);
		kopija.Sort("DESC");
		for (size_t i = 0; i < top; i++)
		{
			cout << "Zahvaljujemo se donatoru " << kopija.GetElement1(i)->GetImePrezime() << "(broj tel: "<<dynamic_cast<Donor*>(kopija.GetElement1(i))->GetTelefon()<<") na ukupnoj kolicini donirane krvi u iznosu od " << kopija.GetElement2(i) << " x" << endl;
		}
	}
};
